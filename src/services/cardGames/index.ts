  ...
  scoringRules: [
    {
      eventType: 'card',
      calculate: ({ event }) => {
        if (event.payload.rank === 'Q' && event.payload.suit === 'spades') return -13;
        if (event.payload.suit === 'hearts') return -1;
        return 0;
      },
...
const spades: ScoringConfig = {
  ...
  scoringRules: [
    ...
    {
      eventType: 'penalty',
      calculate: ({ event }) => event.payload.value || 0,
      appliesTo: (playerId, { event }) => playerId === event.playerId
    }
  ],
  bonusRules: [
    {
      ...
      validate: ({ state, event }) => {
        ...
        round.events.filter(e => e.type === 'bid').forEach(e => bids.set(e.playerId!, e.payload.value));
      ...
      calculate: ({ state, event }) => {
        const round = state.rounds[state.currentRound - 1];
        const bid = round.events.filter(e => e.type === 'bid' && e.playerId === event.payload.playerId)[0]?.payload.value || 0;
        return bid * 10;
      }
    }
  ],
  penaltyRules: [
    {
      ...
      validate: ({ state, event }) => {
        ...
        const round = state.rounds[state.currentRound - 1];
        const bid = round.events.filter(e => e.type === 'bid' && e.playerId === event.payload.playerId)[0]?.payload.value || 0;
        const tricks = round.events.filter(e => e.type === 'trick' && e.playerId === event.payload.playerId).length;
  ...
  scoringRules: [
    {
      ...
      calculate: ({ event }) => {
        ...
        const cards: Array<{ rank: string }> = event.payload.cards || [];
  ...
  penaltyRules: [
    {
      ...
      calculate: ({ event }) => {
        const deadwood = event.payload.deadwoodPoints || 0;
        return deadwood;
      }
    }
  ],
