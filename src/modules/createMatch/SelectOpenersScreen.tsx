...
export const SelectOpenersScreen: React.FC = () => {
  ...
  const resolveSquad = (squadIds: string[] | undefined): import('../../domain/player').Player[] => {
      if (!squadIds) return [];
      return squadIds.map(id => players.find(p => p.id === id)).filter((p): p is import('../../domain/player').Player => !!p);
  };
  ...
  return (
    <PageContainer>
      <PageHeader 
        ...
        backUrl={`/start-match/squads?matchId=${match.id}`}
        action={
            <Button
